      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (tile.terrain === 'water') {
            ...
            // Use enhanced water with animations
            drawEnhancedWaterTile(ctx, screenX, screenY, x, y, fireAnimTimeRef.current, adjacentWater, currentZoom);
...
if (tile.hasMetalDeposit) {
                              ...
                              drawEnhancedMountain(ctx, screenX, screenY, x, y, fireAnimTimeRef.current);
} else if (tile.hasOilDeposit) {
                              // Draw enhanced grass base first
                              drawEnhancedGrassTile(ctx, screenX, screenY, x, y, fireAnimTimeRef.current, currentZoom);
              ...
              // Draw enhanced procedural forest with trees
              drawEnhancedForest(ctx, screenX, screenY, x, y, tile.forestDensity, fireAnimTimeRef.current);
            } else if (tile.building?.type === 'road') {
              // Draw enhanced grass base under roads (roads are drawn on top in second pass)
              drawEnhancedGrassTile(ctx, screenX, screenY, x, y, fireAnimTimeRef.current, currentZoom);
            } else {
              // Regular grass tile with enhanced procedural rendering
              drawEnhancedGrassTile(ctx, screenX, screenY, x, y, fireAnimTimeRef.current, currentZoom);
            }
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          // Draw enhanced beach with animated foam if any adjacent tile is land (and not a dock)
          if (adjacentLand.north || adjacentLand.east || adjacentLand.south || adjacentLand.west) {
            drawEnhancedBeach(ctx, screenX, screenY, x, y, fireAnimTimeRef.current, adjacentLand);
          }
        }
      }

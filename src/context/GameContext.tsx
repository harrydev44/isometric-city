...
export function GameProvider({ children, startFresh = false }: { children: React.ReactNode; startFresh?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!startFresh) {
      ...
      if (saved) {
        ...
        queueMicrotask(() => setHasExistingGame(true));
      } else {
        queueMicrotask(() => setHasExistingGame(false));
      }
    } else {
      queueMicrotask(() => setHasExistingGame(false));
    }
  ...
  useEffect(() => {
    // Keep latest state in a ref for callbacks without breaking render purity.

...
export function CoasterMobileTopBar({ onExit }: CoasterMobileTopBarProps) {
  const { state, setSpeed, setActivePanel, setParkSettings, saveGame } = useCoaster();
      ...
      <Card className="fixed top-0 left-0 right-0 z-40 rounded-none border-x-0 border-t-0 bg-card/95 backdrop-blur-sm safe-area-top">
          ...
          >
            <div className="flex flex-col items-start">
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm" title={weatherDisplay.name}>{weatherDisplay.icon}</span>
              </div>
              <span className="text-muted-foreground text-[10px] font-mono">
                {monthNames[month - 1]} {day}, Yr{year} {timeString}
              </span>
            </div>
            <div className="flex flex-col items-start">
        ...
        <div className="flex items-center justify-between px-3 py-1 border-t border-sidebar-border/50 bg-secondary/30">
          <div className="flex items-center gap-3">
            ...
            {/* Temperature */}
            <div className="flex items-center gap-1">
              <span className="text-[10px]">{Math.round(weather.temperature)}°C</span>
            </div>
          </div>
        ...
        {showTicketSlider && (
          <div className="border-t border-sidebar-border/50 bg-secondary/30 px-3 py-0.5 flex items-center gap-2 text-[10px]">
            <span className="text-muted-foreground whitespace-nowrap">{UI_LABELS.entranceFee}</span>
            <Slider
        ...
        >
          ...
          >
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">{UI_LABELS.monthlyProfit}</span>
            ...
            <div className="p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-muted-foreground">{UI_LABELS.entranceFee}</span>
...
function StatItem({
  ...
  if (format === 'money') {
    displayValue = value >= 1000000 ? `$${(value / 1000000).toFixed(1)}M` : value >= 1000 ? `$${(value / 1000).toFixed(0)}k` : `$${value}`;
  } else if (format === 'percent') {
    displayValue = `${Math.round(value)}%`;
  } else {
    displayValue = isNumber ? value.toLocaleString() : `${Math.round(value)}%`;
  }

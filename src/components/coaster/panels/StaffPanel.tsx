          ...
          <ScrollArea className="h-48 rounded-md border border-border/50">
            <div className="p-3 space-y-2">
              ...
              {staff.map((member) => {
                ...
                const memberTypeLabel = member.type === 'handyman' ? gt('Handyman') :
                  member.type === 'mechanic' ? gt('Mechanic') :
                  member.type === 'security' ? gt('Security') : gt('Entertainer');
                  ...
                  <div key={member.id} className="flex items-start justify-between gap-2 text-sm">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground capitalize">
                        {memberTypeLabel} · {patrolLabel}
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1 text-xs text-muted-foreground">
                      <T><div>$<Num>{member.wage}</Num>/wk</div></T>
                      <T><div>Fatigue <Num>{Math.round((member.fatigue / 255) * 100)}</Num>%</div></T>
                      <div className="flex items-center gap-1">
                        <Button
                          size="sm"

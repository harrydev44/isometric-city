...
export default function StaffPanel({
  ...
  const getStaffTypeLabel = (type: string) => {
    switch (type) {
      case 'handyman': return gt('handyman');
      case 'mechanic': return gt('mechanic');
      case 'security': return gt('security');
      case 'entertainer': return gt('entertainer');
      default: return type;
    }
  };
          ...
          <T>
            <div className="flex items-center justify-between text-xs uppercase tracking-[0.18em] text-muted-foreground">
              ...
              <span className="text-[10px] text-muted-foreground">
                Staff Rooms <Num>{staffRoomCount}</Num>
              </span>
            </div>
          </T>
          {assignmentTarget && (
          ...
          <ScrollArea className="h-48 rounded-md border border-border/50">
            <div className="p-3 space-y-2">
              ...
              {staff.map((member) => {
                ...
                const staffTypeLabel = getStaffTypeLabel(member.type);
                const patrolLabel = member.patrolArea ? gt('Patrol area') : gt('Park-wide');
                ...
                return (
                  <div key={member.id} className="flex items-start justify-between gap-2 text-sm">
                    <div>
                      ...
                      <T>
                        <div className="text-xs text-muted-foreground capitalize">
                          <Var>{staffTypeLabel}</Var> · <Var>{patrolLabel}</Var>
                        </div>
                      </T>
                    </div>
                    <div className="flex flex-col items-end gap-1 text-xs text-muted-foreground">
                      ...
                      <T>
                        <div>Fatigue <Num>{Math.round((member.fatigue / 255) * 100)}</Num>%</div>
                      </T>
                      <div className="flex items-center gap-1">
                        <Button
                          size="sm"

...
import { T, Currency, useGT, useMessages } from 'gt-next';
...
export default function RidePanel({ ride, onClose, onToggleStatus, onPriceChange }: RidePanelProps) {
  ...
  const [prevRidePrice, setPrevRidePrice] = useState(ride.price);

  if (ride.price !== prevRidePrice) {
    setPrevRidePrice(ride.price);
    setPrice(ride.price);
  }
